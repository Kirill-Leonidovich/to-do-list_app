...
const filterSelectValueArray = document.querySelectorAll(
  ".filter-select-value"
);
...
function addNewTask() {
  let newTask = `<div class="list-item">
...
list.addEventListener("click", (event) => {
  ...
  if (event.target.className === "item-btn-task-status__delete") {
    let confirmRemove = confirm(
      "After deleting it will not be possible to resume the task!"
    );
    if (confirmRemove) {
  ...
  if (event.target.className === "item-btn-important") {
    event.target
      .closest(".item-important")
      .classList.toggle("active-important");
  }
});
...
filterSelectCurrent.addEventListener("click", () => {
  ...
  filterSelectValueArray.forEach((filterSelectValueElement) => {
    filterSelectValueElement.addEventListener("click", () => {
      ...
      (filterSelectCurrent && filterSelectBody).classList.remove(
        "active-select"
      );
...
//dark-theme -----
document
  .querySelector(".color-modes-toggle-track")
  .addEventListener("click", () => {
    let arrayDarkTheme = [
      ".todo-body",
      ".color-modes-toggle-track",
      ".title__text",
      ".filter-select",
      ".task-input",
      ".list",
